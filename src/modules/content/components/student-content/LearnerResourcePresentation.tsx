...
import { StudentQuizPlayer } from "@/modules/content/components/student-content/StudentQuizPlayer";
...
type LearnerQuizSummary = Omit<
  NonNullable<StudentContentResourceDetail["quiz"]>,
...
function StoredFileCard({
  ...
  resource: LearnerResourcePresentationData;
}) {
...
function SpecializedResource({
  ...
  switch (resource.type) {
    ...
    case ResourceType.LINK: {
      ...
      const url = link ? safeExternalUrl(link.url) : null;
      if (!link || !url) return <MissingSpecializedContent type={resource.type} />;
      return (
    ...
    case ResourceType.QUIZ:
      if (!resource.quiz) return <MissingSpecializedContent type={resource.type} />;
      if (
        quizInteractive &&
        resource.quiz.attemptsUsed !== undefined &&
        resource.quiz.recentAttempts !== undefined
      ) {
        return (
          <StudentQuizPlayer
            ...
            quiz={{
              ...
              attemptsUsed: resource.quiz.attemptsUsed,
              recentAttempts: resource.quiz.recentAttempts,
            }}
          />
        );
      }
      return (
        <div className="rounded-control border border-violet/20 bg-violet-100/60 p-5 text-sm leading-6 text-[#563ab8] dark:bg-violet/15 dark:text-[var(--student-blue)]">
          Cuestionario de autoevaluación · referencia {resource.quiz.passingScore}%
        </div>
      );
...
export function LearnerResourcePresentation({
  resource,
  moduleTitle,
  headerActions,
  resourcePositionLabel,
  footer,
  titleId,
  ...
  return (
    <article className="overflow-hidden rounded-card border border-line bg-surface shadow-sm dark:border-[var(--student-border)] dark:bg-[var(--student-panel)]">
      <header className="border-b border-line px-5 py-5 dark:border-[var(--student-border)] sm:px-7">
        ...
        {duration || resourcePositionLabel ? (
          <p className="mt-2 flex flex-wrap items-center gap-2 font-meta text-[11px] text-ink-500 dark:text-[var(--student-muted)]">
            ...
            {duration && resourcePositionLabel ? <span aria-hidden="true">·</span> : null}
            {resourcePositionLabel ? <span>{resourcePositionLabel}</span> : null}
          </p>
        ) : null}
      </header>
      ...
      <div className="space-y-5 px-5 py-6 text-[15px] leading-6 text-ink-700 dark:text-[var(--student-muted)] sm:px-7">
        ...
        <SpecializedResource resource={resource} quizInteractive={quizInteractive} />
      </div>

      {footer ? (
        <footer className="border-t border-line px-5 py-4 dark:border-[var(--student-border)] sm:px-7">
          {footer}
        </footer>
      ) : null}
    </article>
