...
export default async function CollaboratorResourceEditPage({ params }: { params: Promise<{ resourceId: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <ContentFormSurface wide>
      {resource.canEdit ? <div className="space-y-4">{resource.publicationStatus === "PUBLISHED" ? <p className="rounded-lg border border-secondary/20 bg-secondary/5 p-3 text-sm text-foreground">Al guardar, los cambios se enviarán automáticamente a revisión. La versión publicada no cambiará hasta que un administrador la apruebe.</p> : null}<EditResourceForm resource={{ ...resource, updatedAt: resource.updatedAt.toISOString() }} closeHref={resourceHref} submitForReview={resource.publicationStatus === "PUBLISHED"} /></div> : <p role="alert" className="rounded-lg bg-amber-500/10 p-4 text-sm text-amber-800 dark:text-amber-200">Retira este recurso de revisión antes de editarlo.</p>}
    </ContentFormSurface>
  </div>;
}
